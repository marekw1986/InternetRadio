...
typedef struct  __attribute__((aligned(2), packed))
{
	...
	//#define WaitForDataByte()		while(!SSPSTATbits.BF);
...
void MACInit(void)
{
    ...
    ENC_SPISTATbits.CKE = 0;// Transmit data on rising edge of clock
...
*/
    ...
    tcpneedReset=0;
	printf("enc reset, spi speed %.1f MHz ENC_SPIBRG=%u SystemClock=%u...\r\n",(float)GetPeripheralClock()/(2*(ENC_SPIBRG+1))/1000000,ENC_SPIBRG,SystemClock);
	j=0;
    do
    {
	//	ClrWdt();
		...
		j++;
		if(j>((SystemClock/400000)))
		{		
			printf("ENC28J60 reset problem! ESTAT=%X\r\n",i);
			j=0;
		}
    } while((i & 0x08) || (~i & ESTAT_CLKRDY));

	printf("enc reset OK in %u loops\r\n",j);
    ...
    //SetLEDConfig(0x3472);
    //OR reverse LED status:
    
    #ifdef ENC_REVERSE_LED
	SetLEDConfig(0x3742);
	#else
	SetLEDConfig(0x3472);
	#endif
...
BOOL MACGetHeader(MAC_ADDR *remote, BYTE* type)
{
    ...
    {
		printf("\r\n\r\nENCSending reset!!!!!\r\n");
		 if(header.NextPacketPointer > RXSTOP) printf("header.NextPacketPointer > RXSTOP\r\n");
		if(((BYTE_VAL*)(&header.NextPacketPointer))->bits.b0) printf("(BYTE_VAL*)(&header.NextPacketPointer))->bits.b0\r\n");
		if(header.StatusVector.bits.Zero)printf("header.StatusVector.bits.Zero\r\n");
		if(header.StatusVector.bits.CRCError)printf("header.StatusVector.bits.CRCError\r\n");
		if(header.StatusVector.bits.ByteCount > 1518u) printf("header.StatusVector.bits.ByteCount > 1518u\r\n");
		if(!header.StatusVector.bits.ReceiveOk) printf("!header.StatusVector.bits.ReceiveOk\r\n");
		printf("\r\n\r\n");
		tcpneedReset=1;
        Reset();
    }
...
static void SendSystemReset(void)
{
    ...
    delay_ms(1);
	// test SPI communication by modyfying EIE register [marek]
	#if 1
	//while(1){
	j=0;
    i =0;
	a = ReadETHReg(EIE).Val;
    while(!(i & 0x40))
	//while(1)
    {
	 //ClrWdt();
     i = ReadETHReg(EIE).Val;
	 	
     WriteReg(EIE, i | (1<<6));
     i = ReadETHReg(EIE).Val;
     j++;
     if(!(i & 0x40) && j>((SystemClock/1000)) )
     {   
         j=0;
         printf("ENC28J60 SPI communication problem EIE=%x\r\n",i);
     } 
    }
	if(i &0x40) printf("ENC28J60 SPI communication OK  EIE=%x %u\r\n",i,j);
	WriteReg(EIE, a);
	delay_ms(100);
	//}
	#endif
